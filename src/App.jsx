  ...
  return (
    <div className="w-full min-h-screen bg-very-light-gray flex justify-center">
      ...
      {delComm.status && (<div className="absolute top-0 bottom-0 right-0 left-0 bg-black/30 flex justify-center items-center backdrop-blur-[5px]">
        <div className="bg-white w-[28%] flex flex-col justify-center p-[30px] gap-[20px] rounded-[10px] animate-summon">
          ...
          <div className="flex justify-between items-center w-full gap-[20px]">
            <button onClick={cancelHandler} className="flex-1 text-white bg-grayish-blue leading-none py-[15px] rounded-[10px]">NO, CANCEL</button>
            <button onClick={deleteHandler} className="flex-1 text-white bg-soft-red leading-none py-[15px] rounded-[10px]">YES, DELETE</button>
          </div>
        </div>
      </div>)}
    </div>
